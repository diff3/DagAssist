...
function DagAssistRetrieveCursorItem(self, event, ...)
	...
	if (GetCursorInfo()) then
		...
		-- DEFAULT_CHAT_FRAME:AddMessage("cursorType: " .. cursorType);
		-- DEFAULT_CHAT_FRAME:AddMessage("cursorData: " .. cursorData);
		-- DEFAULT_CHAT_FRAME:AddMessage(cursorSubType);
...
function DagAssistGetActionInfo(actionType, actionData, actionSubType)
	...
	if actionType == "mount" then
		local creatureName, spellID, icon, active, isUsable, sourceType, isFavorite, isFactionSpecific, faction, hideOnChar, isCollected, mountID = C_MountJournal.GetMountInfoByID(actionData);
		itemName = creatureName;
		itemTexture = icon;
	elseif actionType == "battlepet" then
		local speciesID, customName, level, xp, maxXp, displayID, isFavorite, name, icon, petType, creatureID, sourceText, description, isWild, canBattle, tradable, unique, obtainable = C_PetJournal.GetPetInfoByPetID(actionData)
		itemName = name;
		itemTexture = icon;
...
function DagAssistPickupAction(actionType, actionData, actionSubType)
	if actionType == "mount" then
		...
		--local creatureName, spellID, icon, active, isUsable, sourceType, isFavorite, isFactionSpecific, faction, hideOnChar, isCollected, mountID = C_MountJournal.GetMountInfoByID(actionData);
		-- PickupCompanion(actionSubType, mountID);
		C_MountJournal.Pickup(actionData);
	elseif actionType == "battlepet" then
