...
btnMinimap:RegisterEvent("BAG_UPDATE_DELAYED")
...
if event == "DagAssist" then
  DagAssist:PositionMinimapButton();
  -- It's a missmatch between then game is loaded, and then characters spells
  -- are loaded, this is a quick fix.
...
if (not DA_Vars.Menu) then
  ...
  menuItems = DagAssist.LoadDefaultMenu()
else
...
for index = 1, table.getn(menuItems) do
  ...
  for actionIndex = 1, table.getn(section.Actions) do
    ...
    if action.DA_ActionType == "mount" or action.DA_ActionType == "equipmentset" or action.DA_ActionType == "battlepet" then
      ...
      if action.DA_ActionType == "mount" then
        btnMenuItem:SetScript("OnClick", function(self)
          C_MountJournal.SummonByID(self.Action.DA_ActionData)
        end);

      elseif action.DA_ActionType == "battlepet" then
        btnMenuItem:SetScript("OnClick", function(self)
          C_PetJournal.SummonPetByGUID(self.Action.DA_ActionData)
        end);

      elseif action.DA_ActionType == "equipmentset" then
        btnMenuItem:SetScript("OnClick", function(self)
          local EquipmentID = C_EquipmentSet.GetEquipmentSetID(self.Action.DA_ActionData);
          C_EquipmentSet.UseEquipmentSet(EquipmentID)
        end);
      end
...
function DagAssist:BAG_UPDATE_DELAYED(self, event)
DagAssist:LoadMenu();
end
